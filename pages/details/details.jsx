import {
  View, TouchableOpacity, Image, ScrollView, Linking, FlatList, Dimensions,
} from "react-native";
...
export default function Details({navigation, route}) {
  ...
  const getFavourites = () =>
      AsyncStorage.getItem('favoriteParks').then(
          favoriteParks =>
              setFavourites(JSON.parse(favoriteParks))
      );
  ...
  const toggleFavourite = (idPark) => {
    if (isFavourite) {
      ...
      // AsyncStorage.setItem('favoriteParks', JSON.stringify(favourites));
    } else {
      setFavourites(favourites.filter(id => id !== idPark));
      console.log(favourites);
  ...
  const Item = (item) => (<View style={styles.cardContainer}>
    <Image
        style={styles.parkImage}
        source={{uri: item['title'].url}}
    />
  </View>);
    ...
    {cardDetails ? (<>
          ...
          <View>
            <TouchableOpacity
                onPress={() => {
                  ...
                  toggleFavourite(cardDetails.id)
                }}
                style={styles.backArrow}
            >
              {isFavourite ? (<AntDesign name="star" size={30} color="#fff"/>) : (
      ...
      <ScrollView style={styles.mainInfoContainer}>
        ...
        <View style={styles.valueContainer}>
          ...
          <View
              style={cardDetails.price === "Gratuito" ? styles.free : styles.paid}
          >
            <TouchableOpacity
                onPress={() => {
        ...
        <View style={styles.contact}>
          ...
          <View style={styles.contactIconContainer}>
            <TouchableOpacity
                onPress={() => Linking.openURL(`tel:${cardDetails.telephone}`)}
            >
